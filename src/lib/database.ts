...
async function deleteAllRows(table: string, userId: string) {
  const config = getDatabaseConfig();
...
export function deleteAllHistoryFromDatabase(userId: string) {
  return deleteAllRows("rsswagger_history", userId);
}
